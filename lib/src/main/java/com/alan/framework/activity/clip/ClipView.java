package com.alan.framework.activity.clip;
...
import com.alan.common.utils.bitmap.BitmapUtils;
...
public class ClipView extends RelativeLayout implements OnTouchListener {
    ...
    public static final int MAX_CLOCK_IMG_WIDTH = 640;
    public static final int CLIP_FOR_USER_HEAD = 2;
    ...
    private int mClipWhat = CLIP_FOR_USER_HEAD;
    ...
    // private boolean mNeedCutFunction = true;
     ...
     * @param imagePath
     */
    public void setImage(String imagePath, Rect rect, int clipWhat, int degree) {
        mSrcBitmap = BitmapUtils.getBitmapFromPath(imagePath, getContext());
        setImage(rect, clipWhat, degree);
    }

    public void setImage(Uri uri, Rect rect, int clipWhat, int degree) {
        mSrcBitmap = getBitmapFromUri(uri, getContext());
        setImage(rect, clipWhat, degree);
    }

    private void setImage(Rect rect, int clipWhat, int degree) {
        try {
            if (degree != 1)// degree = 1,不用旋转
                mSrcBitmap = rotateBitmapByDegree(mSrcBitmap, degree);
            mClipWhat = clipWhat;
            ...
            float scaleWidth = ((float) rect.width()) / srcWidth;
            float scaleHeight = ((float) rect.height()) / srcHeight;
            float scale = Math.max(scaleWidth, scaleHeight);
            Matrix matrix = new Matrix();
            matrix.postScale(scale, scale);
            Bitmap bitmap = Bitmap.createBitmap(mSrcBitmap, 0, 0, srcWidth, srcHeight, matrix, true);
            ...
            translate(-(picWidth - clipFrameWidth) / 2, rect.top - (picHeight - clipFrameHeight) / 2);
        } catch (Throwable t) {
            t.printStackTrace();
        }
    }
    ...
    public void rotate(float degree) {
        ...
        mMatrix.postRotate(degree, (rect.left + rect.right) / 2, (rect.top + rect.bottom) / 2);
        mImageView.setImageMatrix(mMatrix);
        mBeforeTrackMatrix.set(mMatrix);
    }
    ...
    private String getBitmapWithCut() {
        ...
        if (rect == null || rect.left >= rect.right || rect.top >= rect.bottom) {
            int w = Math.min(400, mSrcBitmap.getWidth());
            w = Math.min(w, mSrcBitmap.getHeight());
        ...
        matrix.postScale(scale, scale);
        Bitmap bitmap = Bitmap.createBitmap(mSrcBitmap, rect.left, rect.top, rect.right - rect.left,
    ...
    private void fixMatrix(Matrix fromMatrix, Matrix toMatrix) {
        ...
        Rect snapRect = new Rect(mClipViewX, mClipViewY, mClipViewX + mClipViewWidth, mClipViewY + mClipViewHeight);
        ...
        if (isInside(snapRect, new Point(insert.left, insert.top))) {
            ...
            toLeft += snapRect.right - insert.right;
            toTop += snapRect.bottom - insert.bottom;
        } else if (insert.left > snapRect.left && insert.left <= snapRect.right && insert.top <= snapRect.top
                && insert.bottom >= snapRect.bottom) {
            toLeft += snapRect.left - insert.left;
        } else if (hasInsert && insert.right < snapRect.right && insert.right >= snapRect.left
                && insert.top <= snapRect.top && insert.bottom >= snapRect.bottom) {
            toLeft += snapRect.right - insert.right;
        } else if (hasInsert && insert.top > snapRect.top && insert.top <= snapRect.bottom
                && insert.left <= snapRect.left && insert.right >= snapRect.right) {
            toTop += snapRect.top - insert.top;
        } else if (hasInsert && insert.bottom < snapRect.bottom && insert.bottom >= snapRect.top
                && insert.left <= snapRect.left && insert.right >= snapRect.right) {
            toTop += snapRect.bottom - insert.bottom;
        } else {
    ...
    private String saveHeadBitmap(Bitmap bitmap) {
        if (bitmap != null) {
            bitmap = setHeadBitmap(bitmap);
        }

        String path = getContext().getFilesDir().getPath() + getContext().getPackageName() + "temp";
        File dir = new File(path);
        if (!dir.exists()) {
            dir.mkdirs();
        }
        String fileName = path + "/head.png";
        boolean save = BitmapUtils.save(bitmap, fileName);
        return save ? fileName : "";
    }

    private Bitmap setHeadBitmap(Bitmap bm) {
        // 获得图片的宽高
    ...
    private Rect getClipRect() {
        ...
        Point top = getCloest(new Point(scale.left, scale.top), new Point(insert.left, insert.top),
                new Point(insert.left, insert.bottom), new Point(insert.right, insert.top),
                new Point(insert.right, insert.bottom));
        Point bottom = getFarthest(new Point(scale.left, scale.top), new Point(insert.left, insert.top),
                new Point(insert.left, insert.bottom), new Point(insert.right, insert.top),
                new Point(insert.right, insert.bottom));
        ...
        float startXPercent = (float) Math.abs((top.x - scale.left)) / (float) scaleWidth;
        float endXPercent = (float) Math.abs((bottom.x - scale.left)) / (float) scaleWidth;
        float startYPercent = (float) Math.abs((top.y - scale.top)) / (float) scaleHeight;
        float endYPercent = (float) Math.abs((bottom.y - scale.top)) / (float) scaleHeight;
        int degrees = getDegrees();
        if (degrees == 90 || degrees == 270) {
            scaleWidth = Math.abs(scale.bottom - scale.top);
            scaleHeight = Math.abs(scale.right - scale.left);
            startXPercent = (float) Math.abs((top.y - scale.top)) / (float) scaleWidth;
            endXPercent = (float) Math.abs((bottom.y - scale.top)) / (float) scaleWidth;
            startYPercent = (float) Math.abs((top.x - scale.left)) / (float) scaleHeight;
            endYPercent = (float) Math.abs((bottom.x - scale.left)) / (float) scaleHeight;
        }

        Rect resultRec = new Rect((int) (originalWidth * startXPercent), (int) (originalHeight * startYPercent),
                (int) (originalWidth * endXPercent), (int) (originalHeight * endYPercent));
        return resultRec;
    }
    ...
    private Rect getScaleRect() {
        ...
        visibleRect.right = (int) (visibleRect.left + width * values[0] + height * values[1]);
        visibleRect.bottom = (int) (visibleRect.top + height * values[0] - width * values[1]);
        return visibleRect;
    }
    ...
    private Point getFarthest(final Point target, Point... points) {
        ...
        Comparator<Point> comparator = new Comparator<Point>() {
            ...
            public int compare(Point o1, Point o2) {
                double p1_Top = Math.pow(o1.x - target.x, 2) + Math.pow(o1.y - target.y, 2);
                double p2_Top = Math.pow(o2.x - target.x, 2) + Math.pow(o2.y - target.y, 2);
